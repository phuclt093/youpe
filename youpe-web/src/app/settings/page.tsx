...
export default function SettingsPage() {
  ...
  const [counts, setCounts] = useState({ history: 0, later: 0, liked: 0, subs: 0 });
    ...
    <div className="mx-auto max-w-3xl px-4 pb-20 pt-6 sm:px-6">
      ...
      <Section title="Phát video">
        ...
        <Toggle
          ...
          hint="Chuyển sang trang khác thì video co lại thành cửa sổ nhỏ ở góc màn hình thay vì dừng hẳn."
      ...
      <Section title="Phím tắt">
        <div className="grid gap-x-8 gap-y-2 text-sm sm:grid-cols-2">
          {[
            ...
            <div key={k} className="flex items-center justify-between border-b border-yt-border py-1.5">
              <span className="text-yt-sub">{v}</span>
              <kbd className="rounded bg-yt-chip px-2 py-0.5 font-mono text-xs">{k}</kbd>
            </div>
          ))}
        </div>
      </Section>
